...
\begin{frame}
	...
	\begin{columns}%
		...
		\begin{itemize}
			\item Horse hoof
			\item Laminitis \cite{Blaettler2022}%
    			\item Contrast-agent instilled
		\end{itemize}
		\end{column}%
		\begin{column}{0.495\linewidth}%
	        \centering%
...
\begin{tikzpicture}[x=\imagescale,y=-\imagescale]
		\node[anchor=north west, inner sep=0pt, outer sep=0pt] at (0,0) {\includegraphics[width=\imagewidth]{./images/ZMK/45/full/image0000}};
...
\end{tikzpicture}%
			...
			\only<2>{\animategraphics[autoplay,loop,width=\paperwidth,every=\everyframe]{24}{./images/ZMK/45/full/image0}{000}{474}}%
			\only<3>{\animategraphics[autoplay,loop,width=\paperwidth,every=\everyframe]{24}{./images/ZMK/45/full-slices/image0}{000}{466}}%
			};%
	\end{tikzpicture}%
\end{frame}
...
\begin{frame}[label=current]
	...
	\begin{tikzpicture}[remember picture,overlay]%
		\node at (current page.center) [shift={(0,-25pt)}]{%
			\only<1>{\animategraphics[autoplay,loop,width=\paperwidth,every=\everyframe]{24}{./images/ZMK/45/transparent-slices/image0}{000}{474}}%	
			\only<2>{\animategraphics[autoplay,loop,width=\paperwidth,every=\everyframe]{24}{./images/ZMK/45/transparent-slices-pulpa/image0}{000}{474}}%
			};%
	\end{tikzpicture}%
\end{frame}
...
\begin{frame}[label=current]
	...
	\centering%
		...
		\only<4>{%
			\animategraphics[autoplay,height=\imheight,every=\everyframe]{24}{./images/"Chondrules Space Yogita"/NWA1241_HR2/frames/video-0}{101}{200}%
			}%
\end{frame}
